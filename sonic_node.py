...
class SONIC_PreData:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "duration": (
                    "FLOAT",
                    {"default": 10.0, "min": 1.0, "max": 100000000000.0, "step": 0.1},
                ),
                "expand_ratio": (
                    "FLOAT",
    ...
    def sampler_main(
        ...
        if (
            not os.path.exists(audio2bucket_ckpt)
            or not os.path.exists(audio2token_ckpt)
            or not os.path.exists(yolo_ckpt)
        ):
        ...
        audio_path = os.path.join(
            folder_paths.get_input_directory(), f"audio_{audio_file_prefix}_temp.wav"
        )
        ...
        if vae.device != device:
            vae.device = device
        img_latent = vae.encode(tensor_upscale(image, width, height)).to(
        ...
        return (
            {
                ...
                "vae": vae,
                "audio_tensor_list": audio_tensor_list,
                "uncond_audio_tensor_list": uncond_audio_tensor_list,
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "fps": (
                    "FLOAT",
                    {"default": 25.0, "min": 5.0, "max": 120.0, "step": 0.5},
                ),
            }
        }
