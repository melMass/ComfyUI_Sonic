...
def sonic_predata(
    ...
    image_embeds = image_encoder.encode_image(clip_img)[
        "image_embeds"
    ]  # torch.Size([1, 1024])

    if device != torch.device("cpu"):
        image_embeds = (
            image_embeds.clone().detach().to(device, dtype=weight_dtype)
        )  # mps or cuda
    else:
    ...
    for i in range(0, audio_feature.shape[-1], window):
        audio_prompt = wav_enc.encoder(
            audio_feature[:, :, i : i + window], output_hidden_states=True
        ).hidden_states
        last_audio_prompt = wav_enc.encoder(
...
def test(
    ...
    video = decode_latents_(
        video, vae, device, decode_chunk_size=14
    )  # torch.Size([1, 3, 250, 512, 512])
